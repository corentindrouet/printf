...
/*   Updated: 2016/01/27 15:06:33 by cdrouet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	pct_n2(int n, va_list ap, const char *restrict format)
{
	...
	if (ft_strchr(format, 'l') != NULL && (ft_strrchr(format, 'l') == ft_strchr(format, 'l')))
	{
...
void	pct_n(int n, va_list ap, const char *restrict format)
{
	...
	if (ft_strchr(format, 'h') != NULL && (ft_strrchr(format, 'h') == ft_strchr(format, 'h')))
	{
	...
	else if (ft_strchr(format, 'l') || ft_strchr(format, 'j') || ft_strchr(format, 'z'))
		pct_n2(n, ap, format);
	else
	{
		j = (int*)va_arg(ap, int*);
